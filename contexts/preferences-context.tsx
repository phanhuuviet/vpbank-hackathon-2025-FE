...
interface PreferencesContextType {
  ...
  createQuickReply: (data: Omit<QuickReply, "id" | "createdAt" | "updatedAt">) => Promise<QuickReply>
...
export function PreferencesProvider({ children }: { children: ReactNode }) {
  ...
  const updatePreferences = async (updates: Partial<UserPreferences>) => {
    ...
    try {
      const updatedPreferences = await preferencesApi.updateUserPreferences(user.id, updates)
  ...
  const createQuickReply = async (data: Omit<QuickReply, "id" | "createdAt" | "updatedAt">) => {
    if (!user) throw new Error("User not authenticated")

    try {
      const newReply = await quickRepliesApi.createQuickReply({ ...data, userId: user.id })
  ...
  const updateQuickReply = async (id: string, data: Partial<QuickReply>) => {
    try {
      ...
      setQuickReplies((prev) => prev.map((reply) => (reply.id === id ? updatedReply : reply)))
      return updatedReply
    } catch (error) {
