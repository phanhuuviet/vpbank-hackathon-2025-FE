...
const PERMISSIONS = [
  { key: "chat", label: "Chat System", description: "Access to chat and messaging features" },
  { key: "kd", label: "Knowledge Base", description: "Manage knowledge base content" },
  { key: "permission", label: "Permission Management", description: "Manage user permissions" },
  { key: "customer_type", label: "Customer Types", description: "Manage customer segment assignments" },
]
...
export default function PermissionsPage() {
  ...
  const handlePermissionChange = (userId: string, permission: string, checked: boolean) => {
    const currentPermissions = changes[userId] || users.find((u) => u.id === userId)?.permissions || []

    let newPermissions: string[]
    if (checked) {
      newPermissions = [...currentPermissions.filter((p) => p !== permission), permission]
    } else {
  ...
  const getUserPermissions = (userId: string) => {
    return changes[userId] || users.find((u) => u.id === userId)?.permissions || []
  }
                ...
                <thead>
                  <tr className="border-b">
                    ...
                    {PERMISSIONS.map((permission) => (
                      <th key={permission.key} className="text-center py-3 px-4 font-medium text-gray-900">
                  ...
                  {users.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className="border-b hover:bg-gray-50">
                        ...
                        {PERMISSIONS.map((permission) => (
                          <td key={permission.key} className="py-4 px-4 text-center">
                            <Checkbox
                              checked={userPermissions.includes(permission.key)}
                              onCheckedChange={(checked) =>
                                handlePermissionChange(user.id, permission.key, checked as boolean)
                              }
                            />
                          </td>
                        ))}
