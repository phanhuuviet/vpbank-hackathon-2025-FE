...
export function ChatWindow({ conversation, messages, onSendMessage, onShowCustomerProfile }: ChatWindowProps) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b bg-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">{conversation.customerObject.fb_name}</h3>
              <div className="flex items-center space-x-2">
                <Badge variant="outline" className="text-xs">
                  {conversation.customerObject.customer_type || "Unknown"}
                </Badge>
                <span className="text-xs text-gray-500">ID: {conversation.customerObject.fb_id}</span>
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((message) => {
          ...
          const isUser = message.sender_type === "user" && message.sender_id !== "reviewer_1"

          return (
            <div key={message.id} className={cn("flex items-start space-x-2", !isUser && !isBot && "justify-end")}>
              {(isUser || isBot) && (
