...
import type { User, LoginResponse, Conversation, Message, Customer } from "@/types"
...
export const authApi = {
  login: async (username: string, password: string): Promise<LoginResponse> => {
    ...
    if (!user || (password !== "password" && !(username === "admin" && password === "admin123"))) {
...
export const userApi = {
  ...
  setPermission: async (userId: string, permissions: string[]): Promise<void> => {
    await new Promise((resolve) => setTimeout(resolve, 300))
    const userIndex = mockUsers.findIndex((u) => u.id === userId)
    if (userIndex !== -1) {
  ...
  setCustomerTypes: async (userId: string, customerTypes: string[]): Promise<void> => {
    await new Promise((resolve) => setTimeout(resolve, 300))
    const userIndex = mockUsers.findIndex((u) => u.id === userId)
    if (userIndex !== -1) {
...
export const chatApi = {
  ...
  getProfileCustomer: async (customerId: string): Promise<Customer> => {
    await new Promise((resolve) => setTimeout(resolve, 300))
    const conversation = mockConversations.find((c) => c.customerObject.id === customerId)
